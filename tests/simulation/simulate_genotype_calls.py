...
# Used to evaluate population parameter requirements for successful QTL prediction using psQTL.
...
def generate_dummy_genome(genomeLength=10000000):
    ...
    with open("genome.fasta", "w") as fileOut:
        fileOut.write(">chr1\n")
        for i in range(0, genomeLength, MULTILINE_LEN):
...
def main():
    ...
    ########################
    # SIMULATE GENOME FILE #
    ########################
    
    if not os.path.exists("genome.fasta"):
        generate_dummy_genome(args.genomeLength)
